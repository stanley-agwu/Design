...
class TodoList:
    ...
    def get_all_tasks(self, user_id: int) -> List[str]:
        ...
        for task_id in self.user_tasks[user_id]:
            ...
            if not task["completed"]:
                result.append((task["dueDate"], task["description"]))

        result.sort()  # sort by dueDate
        return [description for _, description in result]
    ...
    def complete_task(self, user_id: int, task_id: int) -> None:
        ...
        task = self.tasks[task_id]
        if task["userId"] != user_id or task["completed"]:
            return
