# Design a Todo List (Alternative Solution)
...
"""
Use a global task registry plus per-user sets of task ids.

Because:

-> taskId is globally sequential
-> dueDate values are unique across all tasks, so sorting by due date is straightforward
-> constraints are small, so a clean design is better than overengineering

We store each task once in a dictionary by taskId.
Each task keeps:
...
-> nextTaskId for sequential ids
-> user_tasks[userId] = set/list of task ids belonging to that user

Then:

-> addTask creates a new task and attaches it to the user
-> completeTask validates ownership and completion state
-> getAllTasks filters uncompleted tasks for that user and sorts by dueDate
-> getTasksForTag does the same, but also checks for the tag

Since there are at most 100 calls per method, sorting on demand is fully acceptable.
"""

# Object-Oriented Design: Todo List (Alternative Implementation)
from collections import defaultdict
from typing import List


class Task:
    def __init__(self, user_id: int, description: str, due_date: int, tags: List[str]):
...
class TodoList:
    ...
    def add_task(self, user_id: int, task_description: str, due_date: int, tags: List[str]) -> int:
        task_id = self.next_task_id
        self.next_task_id += 1

        self.tasks[task_id] = Task(user_id, task_description, due_date, tags)
        self.user_tasks[user_id].add(task_id)
        return task_id

    def get_all_tasks(self, user_id: int) -> List[str]:
        arr = []
        for task_id in self.user_tasks[user_id]:
            task = self.tasks[task_id]
            if not task.completed:
                arr.append((task.due_date, task.description))
        arr.sort()
        return [desc for _, desc in arr]

    def get_tasks_for_tag(self, user_id: int, tag: str) -> List[str]:
        arr = []
        for task_id in self.user_tasks[user_id]:
            task = self.tasks[task_id]
    ...
    def complete_task(self, user_id: int, task_id: int) -> None:
        if task_id not in self.tasks:
            return
        task = self.tasks[task_id]
        if task.user_id == user_id and not task.completed:
            task.completed = True

# Complexity Analysis
"""
Let k be the number of tasks owned by the given user.

-> addTask - O(1)
-> completeTask - O(1)
-> getAllTasks - filtering: O(k), sorting: O(k log k)
-> getTasksForTag - filtering: O(k), sorting: O(k log k)
Space
O(n + total_tags) for all stored tasks
"""