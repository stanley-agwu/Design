...
class FileSharing:
    def __init__(self, m: int):
        self.available_ids = []              # reusable IDs
        self.next_id = 1                     # next fresh ID
        self.user_chunks = {}                # user_id -> set of owned chunks
        self.chunk_owners = defaultdict(SortedSet)  # chunk_id -> sorted set of userIDs
    ...
    def leave(self, user_id: int) -> None:
        for chunk in self.user_chunks[user_id]:
            self.chunk_owners[chunk].discard(user_id)

        del self.user_chunks[user_id]
        heappush(self.available_ids, user_id)

    def request(self, user_id: int, chunk_id: int) -> list[int]:
        owners = list(self.chunk_owners[chunk_id])

        if owners:
            # user receives the chunk
            if chunk_id not in self.user_chunks[user_id]:
...
# Complexities
"""
Let U = number of users, C = number of chunks, O = number of owners per chunk (<= U)
Time complexity
join: O(k log U) where k is the number of owned chunks (<= 100)
leave: O(k log U) where k is the number of owned chunks (<= 100)
request: O(O log U) where O is the number of owners of the chunk (<= U)
Space complexity: O(U * k) for user_chunks and O(C * O) for chunk_owners, 
where k is the max number of chunks per user (<= 100) and O is the max number of owners per chunk (<= U).
"""