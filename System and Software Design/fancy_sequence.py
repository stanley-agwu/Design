...
int getIndex(idx) Gets the current value at index idx (0-indexed) of the sequence 
    modulo 109 + 7. If the index is greater or equal than the length of the 
    sequence, return -1.
...
class FancySequence:
    ...
    def get_index(self, idx: int) -> int:
        if idx >= len(self.results):
            return -1
        return (self.results[idx] * self.mul + self.add) % self.MOD
