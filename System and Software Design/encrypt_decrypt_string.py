...
we find an i such that values[i] == s. If there are multiple valid i, we choose any one of them. This means a string could have multiple possible strings it can decrypt to.
...
class Encrypter:
    ...
    def __init__(self, keys: List[str], values: List[str], dictionary: List[str]):
        # char -> encrypted 2-letter string
        self.enc_map = dict(zip(keys, values))
        # self.enc_map = {k: v for k, v in zip(keys, values)}

        # encrypted_string -> how many dictionary words produce it
        self.encrypted_count = {}

        for word in dictionary:
            encrypted = self.encrypt(word)
            if encrypted != "":
                self.encrypted_count[encrypted] = self.encrypted_count.get(encrypted, 0) + 1
...
Let:
K = len(keys) ≤ 26
D = len(dictionary) ≤ 100
L = average dictionary word length
N = length of queried word

Initialization
For each dictionary word, we encrypt it once.
Time: O(total length of dictionary words)
Space: O(total distinct encrypted dictionary strings)

encrypt(word1)
Time: O(len(word1))
Space: O(len(word1)) for output
