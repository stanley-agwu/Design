...
# Idea
"""
Use a one-element cache.
The idea is simple: keep the iterator’s upcoming value preloaded in a 
variable. Then:

1. peek() returns that cached value
2. next() returns the cached value and refreshes the cache from the underlying iterator
3. hasNext() checks whether the cache exists

That gives all operations in O(1) time.
"""

class PeekingIterator:
    def __init__(self, iterator):
        """
        iterator: an existing iterator object that supports:
                  - iterator.next()
                  - iterator.hasNext()
        """
