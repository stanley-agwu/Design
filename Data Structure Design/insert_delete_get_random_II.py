...
class RandomizedCollection:
    ...
    def __init__(self):
        self.arr = []
        self.idxs = defaultdict(set)   # val -> set of indices in arr

    def insert(self, val: int) -> bool:
        not_present = len(self.idxs[val]) == 0

        self.arr.append(val)
        self.idxs[val].add(len(self.arr) - 1)
    ...
    def remove(self, val: int) -> bool:
        ...
        remove_idx = self.idxs[val].pop()
        last_idx = len(self.arr) - 1
        last_val = self.arr[last_idx]

        # If we're not removing the last element itself,
        # move the last element into remove_idx
        if remove_idx != last_idx:
            self.arr[remove_idx] = last_val

            # Update last_val's indices
            self.idxs[last_val].remove(last_idx)
            self.idxs[last_val].add(remove_idx)
