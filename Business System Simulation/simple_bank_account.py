...
class Bank:
    ...
    def __init__(self, balance: list[int]):
        ...
        self.max_account_index = len(balance)

    def _check_account_validity(self, *accounts: int) -> bool:
        for account in accounts:
            if account < 1 or account > self.max_account_index:
                return False
        return True

    def transfer(self, account1: int, account2: int, money: int) -> bool:
        if not self._check_account_validity(account1, account2) \
    ...
    def deposit(self, account: int, money: int) -> bool:
        if not self._check_account_validity(account):
            return False
    ...
    def withdraw(self, account: int, money: int) -> bool:
        if not self._check_account_validity(account) or self.balance[account - 1] < money:
            return False

        self.balance[account - 1] -= money
        return True

# O(1) - Time complexity -> deposit, withdraw, transfer
